...
int main() {
	...
	std::sort(vec.begin(), vec.end(), [](const Ball& a, const Ball& b) { return a.x < b.x; });
	...
	std::sort(vec.begin(), vec.end(), [](const Ball& a, const Ball& b) { return a.y < b.y; });
	...
	for (int i = 0; i < n1 + n2; i++) {
		...
		for (int j = i; j < n1 + n2; j++) {
			if (vec[j].c1) insert(vec[j].x, c1);
			else insert(vec[j].x, -c2);

			// 다음 y 좌표가 같은 경우 최댓값 갱신 X 
			if (j + 1 < n1 + n2 && vec[j].y == vec[j + 1].y) continue;
