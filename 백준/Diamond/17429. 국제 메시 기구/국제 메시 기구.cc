...
std::vector<unsigned int> segtree(MAX * 2, 0);
std::vector<unsigned int> lazy(MAX * 2, 0);
std::vector<unsigned int> lazy2(MAX * 2, 1);
...
std::vector<std::pair<int, int>> euler; //노드 접근
...
unsigned int sum_tree(int start, int end, int left, int right, int point);
int dfs1(int n);
void dfs2(int n);
int count = 1;
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		if (temp == 1) {
			...
			insert_range(euler[index].first, euler[index].second, 0, MAX - 1, 1, var);
		}
		if (temp == 2) {
			int point1, point2, var;
			std::cin >> point1 >> point2 >> var;
			...
			// 다른 체인인 경우
				...
				while (chain_num[point1] != chain_num[point2]) {
					if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
					insert_range(chain[chain_num[point2]].first, renumber[point2], 0, MAX - 1, 1, var);
					point2 = parent[original[chain[chain_num[point2]].first]];
				}

				if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
				insert_range(renumber[point1], renumber[point2], 0, MAX - 1, 1, var);
			}
		}
		if (temp == 3) {
			int index, var;
			std::cin >> index >> var;
			insert_range2(euler[index].first, euler[index].second, 0, MAX - 1, 1, var);
		}
		if (temp == 4) {
			int point1, point2, var;
			std::cin >> point1 >> point2 >> var;
			...
			// 다른 체인인 경우
				...
				while (chain_num[point1] != chain_num[point2]) {
					if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
					insert_range2(chain[chain_num[point2]].first, renumber[point2], 0, MAX - 1, 1, var);
					point2 = parent[original[chain[chain_num[point2]].first]];
				}

				if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
				insert_range2(renumber[point1], renumber[point2], 0, MAX - 1, 1, var);
			}
		}
		if (temp == 5) {
			int index;
			std::cin >> index;
			std::cout << sum_tree(euler[index].first, euler[index].second, 0, MAX - 1, 1) << "\n";
		}
		if (temp == 6) {
			int point1, point2;
			...
			// 다른 체인인 경우
				...
				if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
				sum += sum_tree(renumber[point1], renumber[point2], 0, MAX - 1, 1);
				std::cout << sum << "\n";
			}
		}
		...
		/*
		for (int j = 1; j <= n; j++) {
			std::cout << sum_tree(original[j], original[j], 0, MAX - 1, 1) << " ";
		}
		std::cout << "\n";
		*/
	}
...
void insert_lazy(int left, int right, int point) {
	segtree[point] *= lazy2[point];
	segtree[point] += lazy[point] * (right - left + 1);
	if (left != right) {
		lazy2[point * 2] *= lazy2[point];
		lazy2[point * 2 + 1] *= lazy2[point];
		lazy[point * 2] *= lazy2[point];
		lazy[point * 2 + 1] *= lazy2[point];
