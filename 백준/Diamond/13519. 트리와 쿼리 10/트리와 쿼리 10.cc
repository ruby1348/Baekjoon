	...
	for (int i = 0; i < m; i++) {
		...
		if (temp == 1) {
			...
			if (chain_num[point1] == chain_num[point2]) {
				...
				long long result = std::max((long long)0, sum_tree(left, right, 0, MAX - 1, 1).max);
				std::cout << result << "\n";
			}
			// 다른 체인인 경우
			else {
				...
				std::cout << std::max({ left_node.max, right_node.max, left_node.right + right_node.left, (long long)0 }) << "\n";
			}
		}
