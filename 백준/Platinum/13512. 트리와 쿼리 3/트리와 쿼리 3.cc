...
std::vector<int> segtree(MAX * 2, MAX);
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		if (temp == 1) {
			if (segtree[MAX + renumber[index]] == MAX) insert(renumber[index], renumber[index]);
			else insert(renumber[index], MAX);
		}
		if (temp == 2) {
			// 같은 체인인 경우
			if (chain_num[1] == chain_num[index]) {
				int left = renumber[1];
				int right = renumber[index];
				int min = sum_tree(left, right, 0, MAX - 1, 1);

				if (min == MAX) std::cout << -1 << "\n";
				else std::cout << original[min] << "\n";
			}
			// 다른 체인인 경우
			else {
				int min = MAX;
				while (chain_num[1] != chain_num[index]) {;
					min = std::min(min, sum_tree(chain[chain_num[index]].first, renumber[index], 0, MAX - 1, 1));
					index = parent[original[chain[chain_num[index]].first]];
				}

				min = std::min(min, sum_tree(renumber[1], renumber[index], 0, MAX - 1, 1));
...
void insert(int index, int num) {
	...
	while (point > 0) {
		point /= 2;
		segtree[point] = std::min(segtree[point * 2], segtree[point * 2 + 1]);
	}
}
...
int sum_tree(int start, int end, int left, int right, int point) {
	...
	return std::min(sum_tree(start, end, left, mid, point * 2), sum_tree(start, end, mid + 1, right, point * 2 + 1));
}