...
int main() {
	...
	for (int i = 1; i <= m; i++) {
		...
		if (chain_num[point1] == chain_num[point2]) {
			...
			if(left != right) insert_range(left + 1, right, 0, MAX - 1, 1, i);
		}
		// 다른 체인인 경우
		else {
			while (chain_num[point1] != chain_num[point2]) {
				if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
				insert_range(chain[chain_num[point2]].first, renumber[point2], 0, MAX - 1, 1, i);
				point2 = parent[original[chain[chain_num[point2]].first]];
			}

			if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
			if(point1 != point2) insert_range(renumber[point1] + 1, renumber[point2], 0, MAX - 1, 1, i);
		}
		...
		if (chain_num[point1] == chain_num[point2]) {
			int left = renumber[point1];
			int right = renumber[point2];
			if(left != right) max = std::max(max, sum_tree(left + 1, right, 0, MAX - 1, 1));
		}
		// 다른 체인인 경우
		else {
			while (chain_num[point1] != chain_num[point2]) {
				if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
				max = std::max(max, sum_tree(chain[chain_num[point2]].first, renumber[point2], 0, MAX - 1, 1));
				point2 = parent[original[chain[chain_num[point2]].first]];
			}

			if (renumber[point1] > renumber[point2]) std::swap(point1, point2);
			if(point1 != point2) max = std::max(max, sum_tree(renumber[point1] + 1, renumber[point2], 0, MAX - 1, 1));
		}
		//std::cout << max;
		if (max != i) std::cout << "YES\n";
		else std::cout << "NO\n";
...
long long sum_tree(int start, int end, int left, int right, int point) {
	...
	int mid = (left + right) / 2;
	return std::max(sum_tree(start, end, left, mid, point * 2), sum_tree(start, end, mid + 1, right, point * 2 + 1));
}