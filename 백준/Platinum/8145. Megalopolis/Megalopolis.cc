...
int main() {
	...
	for (int i = 0; i < n + m - 1; i++) {
		...
		if (temp == "W") {
			...
			// 같은 체인인 경우
			if (chain_num[1] == chain_num[point]) {
				int left = renumber[1];
				int right = renumber[point];

				if(left != right) std::cout << sum_tree(left + 1, right, 0, MAX - 1, 1) << "\n";
				if (left == right) std::cout << 0 << "\n";
			}
			// 다른 체인인 경우
			else {
				int sum = 0;
				while (chain_num[1] != chain_num[point]) {
					sum += sum_tree(chain[chain_num[point]].first, renumber[point], 0, MAX - 1, 1);
					point = parent[original[chain[chain_num[point]].first]];
				}

				if(renumber[1] != renumber[point]) sum += sum_tree(renumber[1] + 1, renumber[point], 0, MAX - 1, 1);
